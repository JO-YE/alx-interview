...
def validUTF8(data):
    '''The validUTF8 method is used to validate the data set.'''
    # Initializing a variable to keep track of the number of continuation
    # bytes needed.
    ...
    for byte in data:
        ...
        if byte & 0b11000000 == 0b10000000:
            # If it's a continuation byte, decrease the count of needed
            # continuation bytes.
            num_continuation_bytes -= 1
        else:
            # Checking the number of bytes needed for the current character.
            # leading bits of the byte determine the number of bytes needed.
            if byte & 0b11110000 == 0b11110000:
                ...
                # If the byte doesn't match any valid pattern,
                # it's not a valid UTF-8 encoding.
                return False

            # Checking if the number of continuation bytes needed
            # is greater than the remaining data.
            if num_continuation_bytes > len(data) - 1:
                return False

    # After processing all bytes, if there are still continuation bytes needed
    # it's not a valid encoding.
